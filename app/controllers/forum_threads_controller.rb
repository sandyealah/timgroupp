class ForumThreadsController < ApplicationController
    ...
    def index 
        if params[:search]
            @threads = ForumThread.where('title like?', "%#{params[:search]}%" ).paginate(per_page: 3, page: params[:page])
        else
        @threads = ForumThread.order(sticky_order: :asc).order(id: :desc).paginate(per_page: 3, page: params[:page])
    end 
end
    def show
